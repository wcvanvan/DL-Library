...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        if prod(self._shape) != prod(new_shape):
            raise ValueError(f"NDArray cannot be reshaped into {new_shape}")
    ...
    def broadcast_to(self, new_shape):
        ...
        new_strides = []
        for idx in range(-1, -len(self._shape)-1, -1):
            if new_shape[idx] != self._shape[idx]:
                assert self._shape[idx] == 1
    ...
    def __getitem__(self, idxs):
        ...
        new_offset = 0
        for i, s in enumerate(idxs):
            if s.start >= self._shape[i]:
                raise IndexError(f"Out of boundary at dim {i}, slice start at {s.start} > boundary {self._shape[i] - 1}")
            length = (min(s.stop, self._shape[i]) - s.start - 1) // s.step + 1
            new_shape.append(length)
            
            new_strides[i] *= s.step
            new_offset += self._strides[i] * s.start
        return NDArray.make(tuple(new_shape), tuple(new_strides), device=self._device, handle=self._handle, offset=new_offset)
    ...
    def flip(self, axes):
        if axes is None:
            axes = range(len(self._shape))
        new_strides = list(self._strides)
        new_offset = 0
        for axis in axes:
            stride = self._strides[axis]
            new_offset += (self._shape[axis]-1) * stride
            new_strides[axis] = -stride
        new_array = NDArray.make(self._shape, new_strides, device=self._device, handle=self._handle, offset=new_offset).compact()
    ...
    def pad(self, axes):
        ...
        for idx, pad in enumerate(axes):
            new_shape[idx] = new_shape[idx] + pad[0] + pad[1]
        new_array = NDArray.make(new_shape, device=self.device)
        new_array.fill(0)
        slices = [slice(pad[0], pad[0]+self.shape[idx]) for idx, pad in enumerate(axes)]
