...
def xavier_uniform(fan_in, fan_out, shape: Optional[tuple], gain=1.0, **kwargs):
    bound = gain * math.sqrt(6/(fan_in+fan_out))
    return rand(*shape, low=-bound, high=bound, **kwargs)


def xavier_normal(fan_in, fan_out, shape: Optional[tuple], gain=1.0, **kwargs):
    std = gain * math.sqrt(2/(fan_in+fan_out))
    return randn(*shape, std=std, **kwargs)


def kaiming_uniform(fan_in, fan_out, shape: Optional[tuple], nonlinearity="relu", **kwargs):
...
def kaiming_normal(fan_in, fan_out, shape: Optional[tuple], nonlinearity="relu", **kwargs):
    assert nonlinearity == "relu", "Only relu supported currently"
    gain = math.sqrt(2)
